  ...
  return (
    <Link href='/' passHref legacyBehavior>
      <div className='flex flex-nowrap items-center cursor-pointer font-extrabold'>
        <span className={`iconfont icon-caidan text-2xl mr-2 ${textWhite ? 'text-white' : 'text-black dark:text-white'}`}></span>
        <div id='logo-text' className='group rounded-2xl flex-none relative'>
          <div className={`logo group-hover:opacity-0 opacity-100 visible group-hover:invisible text-lg my-auto rounded dark:border-white duration-200 ${textWhite ? 'text-white' : ''}`}>
            {siteConfig('TITLE')}
          </div>
